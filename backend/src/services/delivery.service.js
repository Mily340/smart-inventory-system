...
export const listDeliveries = async ({ branchId, riderId, orderId }) => {
  ...
  // branchId is on order, not delivery, so filter via relation
  const deliveries = await prisma.delivery.findMany({
    where,
    include: {
      order: { include: { branch: true, distributor: true, items: { include: { product: true } } } },
      rider: true,
  ...
  return deliveries.filter((d) => d.order?.branchId === branchId);
};
...
export const createDelivery = async ({ orderId, riderId, destinationAddress }, user) => {
  ...
  return prisma.$transaction(async (tx) => {
    const order = await tx.order.findUnique({
      where: { id: orderId },
      include: { branch: true, distributor: true },
    });
    if (!order) throw new ApiError(400, "Invalid orderId");

    // only allow delivery creation for orders that are ready
    if (!["APPROVED", "PACKED", "DISPATCHED"].includes(order.status)) {
      throw new ApiError(400, "Order must be APPROVED or PACKED (or DISPATCHED) to create delivery");
    }

    const existingDelivery = await tx.delivery.findUnique({ where: { orderId } });
    if (existingDelivery) throw new ApiError(409, "Delivery already exists for this order");

    const rider = await tx.user.findUnique({ where: { id: riderId } });
    if (!rider) throw new ApiError(400, "Invalid riderId");
    if (rider.role !== "DELIVERY_RIDER") throw new ApiError(400, "Selected user is not a DELIVERY_RIDER");

    // code generation (assumes Delivery has code field)
    ...
    const code = nextCode("DL", lastNum);
    ...
    const delivery = await tx.delivery.create({
      ...
      include: {
        order: { include: { branch: true, distributor: true } },
        rider: true,
      },
    });

    // notification: branch + rider
    await notify(tx, {
      branchId: order.branchId,
      title: "Delivery assigned",
      message: `Delivery ${delivery.code || delivery.id} assigned for Order ${order.code || order.id} to rider ${rider.fullName}`,
    });
    await notify(tx, {
      userId: riderId,
      title: "New delivery assigned",
      message: `You have been assigned Delivery ${delivery.code || delivery.id} for Order ${order.code || order.id}`,
    });
...
export const updateDeliveryStatus = async (id, { status }, user) => {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // rider can update only own delivery; admins/managers can update any
    ...
    await notify(tx, {
      ...
      message: `Delivery ${updated.code || updated.id} status is now ${updated.status} (Order ${updated.order.code || updated.order.id})`,
    });
    await notify(tx, {
