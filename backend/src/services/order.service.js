...
const ensureUserBranchAccess = (user, branchId) => {
  if (!BRANCH_SCOPED_ROLES.includes(user?.role)) return;
...
export const createOrder = async ({ distributorId, branchId, items }, user) => {
  ...
  ensureUserBranchAccess(user, branchId);
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!distributor) {
      throw new ApiError(400, "Invalid distributorId");
    }
    ...
    for (let idx = 0; idx < items.length; idx++) {
      ...
      if (!inv) {
        throw new ApiError(400, `No stock record for ${product.name} in this branch`);
      }
